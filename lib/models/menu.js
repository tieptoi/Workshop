...
MenuSchema.path('title').validate(function (text) {
    return text !== null && typeof text !== 'undefined' && text.trim().length >= 1;
}, 'Title is empty');
...
MenuSchema.path('link').validate(function (link) {
    return (link !== null && typeof link !== 'undefined' && link.trim().length >= 1);
}, 'Link is invalid');

//Link Menu SortOrder=========================
MenuSchema.path('sortOrder').validate(function (num) {
    return (num !== null && typeof num !== 'undefined' && validator.isNumeric(num));
